...
import { Employee, Redirect } from "@/lib/db/schema";
...
export type CeoTab = "analytics" | "users" | "employees";
...
export function CeoCommandView({ initialTab = "analytics" }: CeoCommandViewProps) {
  const [activeTab, setActiveTab] = useState<CeoTab>(initialTab);
  const [overview, setOverview] = useState<StatsOverview | null>(null);
  const [topLinks, setTopLinks] = useState<Array<Redirect & { periodClicks: number }>>([]);
  ...
  useEffect(() => {
    if (pathname === "/ceo/users" || pathname === "/ceo/creators") {
      setActiveTab("users");
    } else if (pathname === "/ceo/employees" || pathname === "/ceo/team") {
      setActiveTab("employees");
    } else if (pathname === "/ceo/analytics" || pathname === "/ceo/worldwide") {
  ...
  const switchTab = (tab: CeoTab) => {
    setActiveTab(tab);
    let targetPath = "/ceo";
    if (tab === "analytics") targetPath = "/ceo/analytics";
    else if (tab === "users") targetPath = "/ceo/users";
    else if (tab === "employees") targetPath = "/ceo/employees";
  ...
  const fetchCeoData = useCallback(async () => {
    try {
      ...
      const [statsRes, employeesRes] = await Promise.all([
        fetch(statsUrl, { cache: "no-store", headers: { "Cache-Control": "no-cache" } }),
        fetch(`/api/ceo/employees?t=${Date.now()}`, { cache: "no-store", headers: { "Cache-Control": "no-cache" } }),
      ]);
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-12 py-8 sm:py-10 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-5 rounded-2xl border border-neutral-800 bg-neutral-900/60 shadow-md">
            <div className="flex items-center justify-between">
              <p className="text-xs text-neutral-400 font-medium">Worldwide Traffic</p>
          ...
          <div className="p-5 rounded-2xl border border-neutral-800 bg-neutral-900/60 shadow-md">
            <div className="flex items-center justify-between">
              <p className="text-xs text-neutral-400 font-medium">Creators & Users</p>
              <div className="w-8 h-8 rounded-xl bg-neutral-800 flex items-center justify-center text-white">
          ...
          <div className="p-5 rounded-2xl border border-neutral-800 bg-neutral-900/60 shadow-md">
            <div className="flex items-center justify-between">
              <p className="text-xs text-neutral-400 font-medium">Total Links</p>
              <div className="w-8 h-8 rounded-xl bg-neutral-800 flex items-center justify-center text-white">
        ...
        {activeTab === "analytics" && (
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <div className="p-4 rounded-2xl border border-neutral-800 bg-neutral-900/60 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 shadow-sm p-6 sm:p-8 space-y-5">
              <div className="flex items-center justify-between pb-4 border-b border-neutral-800">
                <div>
                  <h3 className="text-base font-bold text-white">Top Performing Links Worldwide</h3>
                ...
                <div className="space-y-2.5">
                  {topLinks.map((link, idx) => {
                    ...
                    return (
                      <div
                        key={link.id}
                        className="p-4 rounded-2xl border border-neutral-800/80 bg-neutral-950/60 flex items-center justify-between gap-3"
                      >
                        <div className="min-w-0 flex items-center gap-3.5">
                          <div className="w-7 h-7 rounded-xl bg-neutral-800 text-white flex items-center justify-center font-bold text-xs flex-shrink-0 font-mono">
                            #{idx + 1}
                          </div>
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          <div className="text-[10px] text-neutral-500">clicks in range</div>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((u) => (
                      <tr key={u.username} className="hover:bg-neutral-800/40 transition">
                        ...
                        <td className="py-4 px-5 text-center font-mono font-medium text-neutral-300">
                          {u.activeLinks}
                        </td>
        ...
        {activeTab === "employees" && (
          <div className="space-y-6 animate-in fade-in duration-200">
            <div className="p-6 rounded-3xl border border-neutral-800 bg-neutral-900/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
              <div className="flex items-center gap-3.5">
                <div className="w-12 h-12 rounded-2xl bg-white text-black flex items-center justify-center font-bold text-lg shadow-md flex-shrink-0">
                  <ShieldCheck className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-white">Employee & Team Access</h3>
                  <p className="text-xs text-neutral-400 mt-0.5">
                    Authorize employees, delegate moderation tasks, and manage platform permissions
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-4 py-2.5 text-xs font-semibold text-black bg-white hover:bg-neutral-200 rounded-xl transition shadow-sm cursor-pointer"
              >
            ...
            {/* Employees Table */}
            <div className="rounded-3xl border border-neutral-800 bg-neutral-900/60 shadow-sm overflow-visible">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-neutral-800 bg-neutral-900 text-[11px] uppercase tracking-wider text-neutral-400 font-semibold">
                  ...
                  {employeesList.map((emp) => {
                    ...
                    return (
                      <tr key={emp.id} className="hover:bg-neutral-800/40 transition relative">
                        ...
                        <td className="py-4 px-5">
                          <span className="text-xs font-medium text-neutral-300">{emp.role}</span>
                        </td>
                        ...
                        <td className="py-4 px-5 text-right relative">
                          <div className="inline-block text-left">
                            ...
                            {isMenuOpen && (
                              <div className="absolute right-4 top-12 z-50 w-44 rounded-2xl bg-neutral-900 border border-neutral-800 p-1.5 shadow-2xl animate-in fade-in zoom-in-95 duration-150 text-left">
                                <button
                                  type="button"
                                  onClick={() => {
                                  ...
                                  className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-neutral-300 hover:text-white hover:bg-neutral-800 rounded-xl transition text-left cursor-pointer"
                                >
                                ...
                                <button
                                  ...
                                  className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-red-400 hover:text-red-300 hover:bg-red-950/40 rounded-xl transition text-left cursor-pointer"
                                >
